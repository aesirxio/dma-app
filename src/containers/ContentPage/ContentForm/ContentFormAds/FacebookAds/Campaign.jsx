...
const Campaign = observer((props) => {
  ...
  const campaignData = viewModel.formPropsData[CHANNEL_ADS_FACEBOOK].campaign ?? {};
      ...
      fields: [
        {
          ...
          changed: (event) => {
            viewModel.setFromCampaignData('name', event.target.value, CHANNEL_ADS_FACEBOOK);
          },
        },

        {
          ...
          startField: {
            ...
            changed: (date) => {
              viewModel.setFromCampaignData('startdate', date, CHANNEL_ADS_FACEBOOK);
            },
            required: true,
            validation: 'required',
          },
          endField: {
            ...
            // validation: 'required',
            changed: (date) => {
              viewModel.setFromCampaignData('enddate', date, CHANNEL_ADS_FACEBOOK);
            },
          },
        },
        {
          ...
          validation: 'required',
          changed: (event) => {
            viewModel.setFromCampaignData('budget', event.value, CHANNEL_ADS_FACEBOOK);
          },
        },
      ],
    };
  ...
  const handleNext = () => {
    if (validator.allValid()) {
      if (ContentUtils.hasAdvertisingAds(channelData, CHANNEL_ADS_GOOGLE)) {
        viewModel.checkValidateStep(CHANNEL_ADS_FACEBOOK);
        if (viewModel.validateGG) {
